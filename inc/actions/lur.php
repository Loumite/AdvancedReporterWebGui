...
class Lur extends Database
{
  ...
  public function __construct()
  {
    ...
    if($this->checkSession() == true)
    {
      ...
      $select_reports = $dbh->connect()->query("SELECT * FROM $this->_table WHERE open='1' AND ticketManager='none'");
      ...
      while($display_reports = $select_reports->fetch_array())
      {
        $this->report_id += 1;
	      ...
	      $raw_reports[$this->report_ID]['action'] = "<a href='".$this->setting("website_scheme").$this->setting("website_domain")."/module/report/".$this->report_ID."' class='btn btn-primary btn-sm'><i class='fas fa-cog'></i> Manage</a>";
      }
